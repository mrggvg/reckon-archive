...
| — | Continuous tax monitoring and year-end reconciliation | **Implemented.** Two separate engines — contributions from the calendar and registration date, income tax from revenue actually received — both pure functions in the shared package, verified to the cent against two real PODO-OPSVZ filings. With them: a month-by-month contribution schedule that accepts real filings in place of estimates, a cumulative revenue trajectory against the statutory thresholds, a ledger of remittances, year-end reconciliation against the odločba, UPN payment codes for each obligation, and an effective hourly rate after both obligations are paid |
